...
void SobMainWin::Gauss_blur( bool d )
{
	...
	uint gknl[][5] = { { 2, 4, 5, 4, 2 }, { 4, 9, 12, 9, 4 }, { 5, 12, 15, 12, 5 }, { 2, 4, 5, 4, 2 }, { 4, 9, 12, 9, 4 } };
	const uint sum = 159;
	...
	for ( int y = 0; y < out_im -> height(); y++ ) {
		for ( int x = 0; x < out_im -> width(); x++ ) {
			...
			if ( (y < 2) or (y + 2 >= out_im -> height()) ) {
				...
				for ( int i = -2; i <= 2; i++ ) {
					for ( int j = -2; j <= 2; j++ ) {
						px += (qRed(out_im -> pixel(x + i, y + j)) * gknl[i + 2][j + 2]);
					}
				}
				px /= sum;
			}
...
boost::shared_ptr<SobMainWin::hought_t> SobMainWin::Hough_tm( bool d, uint rad )
{
	...
	std::cout << IMSIZE * sizeof(uint) / 1024.0 / 1024.0 << "MB (" << IMSIZE << " * " << sizeof(uint) << ")" << std::endl;
		...
		for ( int x = 0; x < out_im -> width(); x++ ) {
			if ( qRed(out_im -> pixel(x, y)) == 0 ) {
				for ( uint16_t theta = 0; theta < 360; ++theta ) {
					...
					if ( (x0 > 0) and (x0 < out_im -> width()) and (y0 > 0) and (y0 < out_im -> height()) ) {
	...
	if ( !d ) {
		...
		for ( int i = 0; i < MAXELS; i++ ) {
			std::cout << i << ": " << (it -> get<0> ()) << " " << (it -> get<1> ()) << ": " << (it -> get<2> ()) << std::endl;
			if ( it -> get<2> () > 250 ) {
				qp.setPen(QColor("red"));
			}
			else {
...
boost::shared_ptr<SobMainWin::grad_t> SobMainWin::Make_grads( bool )
{
	...
	const uint mx = Find_eyeline_el(out_im -> height() / 4, out_im -> height() / 4 * 2, gt_x);
	const uint pct = static_cast<int> (((out_im -> height()) * (XTOLPCT / 100.0)));
	const int up = mx + pct;
...
void SobMainWin::Canny_ed( bool d )
{
	...
	std::vector<std::vector<uint8_t> > edir(out_im->width(), std::vector<uint8_t>(out_im->height(), 0)); //edge direction
	std::vector<std::vector<uint64_t> > grad(out_im->width(), std::vector<uint64_t>(out_im->height(), 0)); //gradient value
	...
	for ( size_t x = 1; x < edir.size() - 1; ++x ) {
		for ( size_t y = 1; y < edir[x].size() - 1; ++y ) {
			...
			grad[x][y] = static_cast<ulong> (std::sqrt(std::pow((double) gx, 2.0) + std::pow(gy, 2.0)));
	...
	for ( size_t x = 0; x < edir.size(); ++x ) {
		for ( size_t y = 0; y < edir[x].size(); ++y ) {
			...
			if ( x > 0 && y > 0 && (grad[x][y] > thd1) ) {
				...
				{
					case 0:
						canny_edge_trace(tmpi, 0, x, y, std::make_pair(0, 1), edir, grad, thd1, thd2);
						break;
					case 45:
						canny_edge_trace(tmpi, 45, x, y, std::make_pair(1, 1), edir, grad, thd1, thd2);
						break;
					case 90:
						canny_edge_trace(tmpi, 90, x, y, std::make_pair(1, 0), edir, grad, thd1, thd2);
						break;
					case 135:
						canny_edge_trace(tmpi, 135, x, y, std::make_pair(1, -1), edir, grad, thd1, thd2);
						break;
					default:
	...
	for ( size_t x = 0; x < edir.size(); ++x ) {
		for ( size_t y = 0; y < edir[x].size(); ++y ) {
			...
			if ( qRed(tmpi.pixel(x, y)) == 255 ) {
				...
				{
					case 0:
						canny_supr_nonmax(tmpi, 0, x, y, std::make_pair(1, 0), edir, grad, thd2);
						break;
					case 45:
						canny_supr_nonmax(tmpi, 45, x, y, std::make_pair(1, -1), edir, grad, thd2);
						break;
					case 90:
						canny_supr_nonmax(tmpi, 90, x, y, std::make_pair(0, 1), edir, grad, thd2);
						break;
					case 135:
						canny_supr_nonmax(tmpi, 135, x, y, std::make_pair(1, 1), edir, grad, thd2);
						break;
					default:
						break;
				}
...
void SobMainWin::canny_edge_trace( QImage &qi, uint8_t dir, uint64_t row, uint64_t col, std::pair<int8_t, int8_t> shift,
		const std::vector<std::vector<uint8_t> > &edirs, const std::vector<std::vector<uint64_t> > & grads, const uint64_t, const uint64_t t2 )
{
	uint64_t lrow, lcol;
	lrow = lcol = 0;
	bool doit = true;
	...
	doit = (not (lrow == std::numeric_limits<uint64_t>::max() || lcol == std::numeric_limits<uint64_t>::max()));
...
void SobMainWin::canny_supr_nonmax( QImage &qi, uint8_t dir, uint64_t row, uint64_t col, std::pair<int8_t, int8_t> shift, const std::vector<
		std::vector<uint8_t> > &edirs, const std::vector<std::vector<uint64_t> > & grads, const uint64_t )
{

	uint64_t lrow, lcol;
	lrow = lcol = 0;
	bool doit = true;
	std::vector<std::vector<float> > nmax(qi.width(), std::vector<float>(3, 0.0));
	...
	while (doit && (edirs[lrow][lcol] == dir) && (qRed(qi.pixel(lrow, lcol)) == 0)) {
		lrow = canny_et_mkrowcol(qi.width(), lrow, shift.first, doit);
		lcol = canny_et_mkrowcol(qi.height(), lcol, shift.second, doit);
...
uint64_t SobMainWin::canny_et_mkrowcol( uint64_t max, uint64_t curr, int8_t shift, bool & eend ) const
{
...
boost::shared_ptr<SobMainWin::vpf_s_t> SobMainWin::Vpf_simple( const QPoint & pos, const QPoint &, double wdh, double hgt )
{
	this -> setCursor(Qt::WaitCursor);
	...
	gradarr_t vh(new gradarr_t::element_type[static_cast<size_t> (std::ceil(hgt))]);
	gradarr_t vv(new gradarr_t::element_type[static_cast<size_t> (std::ceil(wdh))]);

	std::fill(vh.get(), vh.get() + static_cast<size_t> (hgt), 0 );
	std::fill(vv.get(), vv.get() + static_cast<size_t> (wdh), 0 );

	boost::shared_ptr<vpf_s_t> vpf(new vpf_s_t(vh, vv, QPoint()));

	QPoint mypos = pos;

	uint64_t srednia = 0, suma = 0;
	double maxv = 0/*, odch = 0*/;


	std::cout << "vpf x: " << pos.x() << ", y: " << pos.y() << ", wdh: " << wdh << "hgt: " << hgt << std::endl;

	//h
	for ( uint i = mypos.y(); i < mypos.y() + hgt; ++i ) {
		for ( uint j = mypos.x(); j < mypos.x() + wdh; ++j ) {
			srednia += qRed(out_im -> pixel(j, i));
		}

		srednia /= static_cast<uint> (wdh);

		for ( uint j = mypos.x(); j < mypos.x() + wdh; ++j ) {

			int pxval = qRed(out_im -> pixel(j, i));
			suma += std::pow((pxval - srednia), 2);
		}

		vh[i - std::floor(mypos.y())] = static_cast<int> (suma / hgt);
	...
	srednia = suma = maxv = 0;
	//v
	for ( uint i = mypos.x(); i < mypos.x() + wdh; ++i ) {
		for ( uint j = mypos.y(); j < mypos.y() + hgt; ++j ) {
			srednia += qRed(out_im -> pixel(i, j));
		}
		...
		vv[i - std::floor(mypos.x())] = static_cast<int> (suma / hgt);
	...
	std::cout << "vpf qx: " << vpf->get<2> ().x() << "; qy: " << vpf->get<2> ().y() << std::endl;
...
/*
 boost::shared_ptr<SobMainWin::vpf_t> SobMainWin::Vpf(const eyeloc & eloc)
 {
 boost::shared_ptr<vpf_t> vpf(new vpf_t);


 vpf->get<0>().reset(new gradarr_t::element_type[static_cast<size_t>(eloc->get<3>())]);
 vpf->get<1>().reset(new gradarr_t::element_type[static_cast<size_t>(eloc->get<2>())]);
