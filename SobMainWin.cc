...
SobMainWin::SobMainWin() :
	mwin_ui(new Ui::MainWindow()), sobel(false), bin(false), XTOLPCT(5.0),
			YTOLPCT(7.5) {
...
void SobMainWin::connects() {
	connect(mwin_ui -> actionWczytaj, SIGNAL(triggered ( bool ) ), this,
			SLOT(Load_file(bool)));
	connect(mwin_ui -> actionZapisz, SIGNAL(triggered ( bool ) ), this,
			SLOT(Save_file(bool)));
	connect(mwin_ui -> actionDajesz, SIGNAL(triggered ( bool ) ), this,
			SLOT(Sobel_op(bool)));
	connect(mwin_ui -> actionSzaro, SIGNAL(triggered ( bool ) ), this,
			SLOT(To_gray(bool)));
	connect(mwin_ui -> actionGradient, SIGNAL(triggered ( bool ) ), this,
			SLOT(Disp_grad(bool)));
	connect(mwin_ui -> actionAvg_blur, SIGNAL(triggered ( bool ) ), this,
			SLOT(Avg_blur(bool)));
	connect(mwin_ui -> actionGauss, SIGNAL(triggered ( bool ) ), this,
			SLOT(Gauss_blur(bool)));
	connect(mwin_ui -> actionFiltr_medianowy, SIGNAL(triggered ( bool ) ),
			this, SLOT(Median_fr(bool)));
	connect(mwin_ui -> actionBinaryzacja_2, SIGNAL(triggered ( bool ) ), this,
			SLOT(Otsus_bin(bool)));
	connect(mwin_ui -> actionLame, SIGNAL(triggered ( bool ) ), this,
			SLOT(Lame_bin(bool)));
	connect(mwin_ui -> actionHough, SIGNAL(triggered ( bool ) ), this,
			SLOT(Hough_tm(bool)));
	connect(mwin_ui -> actionCanny, SIGNAL(triggered ( bool ) ), this,
			SLOT(Canny_ed(bool)));

	connect(mwin_ui -> actionGo, SIGNAL(triggered ( bool ) ), this,
			SLOT(Do_auto(bool)));

	connect(mwin_ui -> actionSegmentacja_koloru, SIGNAL(triggered ( bool ) ),
			this, SLOT(Face_find_cs(bool)));

	connect(mwin_ui -> actionMaska_gradient_w, SIGNAL(triggered ( bool ) ),
			this, SLOT(Disp_feat(bool)));
	connect(mwin_ui -> actionOczy_houghem, SIGNAL(triggered ( bool ) ), this,
			SLOT(Disp_eyes_ht(bool)));
	connect(mwin_ui -> actionOczy_sobelem, SIGNAL(triggered ( bool ) ), this,
			SLOT(Disp_eyes_sob(bool)));

	// connect(mwin_ui -> actionPrzytnij, SIGNAL(triggered ( bool ) ), this, SLOT(Crop_face(bool)) );
	connect(mwin_ui -> actionPrzytnij, SIGNAL(triggered ( bool ) ), this,
			SLOT(Crop_face_manual(bool)));

	connect(mwin_ui -> verticalSlider, SIGNAL( sliderMoved ( int ) ), this,
			SLOT(Set_gamma_lbl(int)));
}
...
void SobMainWin::Disp_grad(bool) {
	...
	const int maxx = std::max_element(g -> get<0> ().get(),
			g -> get<0> ().get() + out_im -> height()) - g -> get<0> ().get();
	const int maxy = std::max_element(g -> get<1> ().get(),
			g -> get<1> ().get() + out_im -> width()) - g -> get<1> ().get();
...
void SobMainWin::Disp_feat(bool d) {
	...
	qp.setBrush(QBrush(QColor("red")));
	qp.setOpacity(0.05);
	qp.drawRect(0, g -> get<0> ()[0] - (g -> get<2> ()), tmp.width(), g -> get<
			2> () * 2);
	qp.setOpacity(1);
...
void SobMainWin::Crop_face_manual(bool) {
	kropuj = 1;
	mwin_ui -> label_2 -> setCursor(Qt::CrossCursor);
}

void SobMainWin::mouseReleaseEvent(QMouseEvent * evt) {
	if (childAt(evt->pos()) and childAt(evt->pos()) == mwin_ui->label_2)
	{
		if (kropuj == 1) {
				QMessageBox::critical(this, "b", "b");
		}
	}
}
